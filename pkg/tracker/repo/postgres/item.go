...
func (i Item) GetAllItems() (items []Item, err error) {
	...
	query := `
	SELECT i.id, i.name, i.url, last_price, lowest_price, status, e.site_url as ecommerce_url, e.name as ecommerce_name, e.discount_price_selector, e.original_price_selector, e.name_selector, i.date_created
	FROM item i
